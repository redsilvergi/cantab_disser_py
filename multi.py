# -*- coding: utf-8 -*-
"""
Created on Wed Mar  6 02:20:08 2024
...
try:
    for i in range(len(csvnms3)):
        data = pd.read_csv(f"./csv/2023/sale/multi/{csvnms3[i]}.csv", encoding='ansi', skiprows=15, low_memory=False)
        datar = pd.read_csv(f"./csv/2023/rent/multi/{csvnms3[i]}.csv", encoding='ansi', skiprows=15, low_memory=False)

        ##delete unnecessary columns -------------------------------------------------

        # data = pd.read_csv(
        #     "./csv/2023/sale/multi/sol.csv",
        #     encoding="ansi",
        #     skiprows=15,
        #     low_memory=False,
        # )
        # datar = pd.read_csv(
        #     "./csv/2023/rent/multi/sol.csv",
        #     encoding="ansi",
        #     skiprows=15,
        #     low_memory=False,
        # )

        sale_del_col = ['NO', '시군구', '번지', '본번', '부번',  '대지권면적(㎡)', '계약년월',
               '계약일',  '매수자', '매도자',  '해제사유발생일', '거래유형',
               '중개사소재지', '등기일자']
        rent_del_col = ['NO', '시군구', '번지', '본번', '부번',  '전월세구분',  '계약년월', '계약일',
        ...
        ##export az-------------------------------------------------------
        az.to_csv(f"./res_tmp/az_{csvnms3[i]}_multi.csv", index=False)
        print(f"./res_tmp/az_{csvnms3[i]}_multi.csv exported")
